...
class SpacyNER:
    def __init__(self, spacy_model):
        self.bg_spacy_model = spacy.load(spacy_model)
    ...
    def batch_ner(self, hash_id_to_passage, max_workers):
        ...
        docs_list = self.bg_spacy_model.pipe(passage_list, batch_size=batch_size)
        passage_hash_id_to_entities = {}
        sentence_to_entities = defaultdict(list)
        for idx, doc in enumerate(docs_list):
            passage_hash_id = list(hash_id_to_passage.keys())[idx]
            single_passage_hash_id_to_entities, single_sentence_to_entities = self.extract_entities_sentences(doc,
                                                                                                              passage_hash_id)
    ...
    def question_ner(self, question: str):
        doc = self.bg_spacy_model(question)
